...
外部集成方对 Twin 公开 REST API 做了一轮实测，发现三项会阻断 `present_options` 人机交互回路的问题。本仓库提交 bug 报告给 Twin 工程团队。

## 核心问题速览

1. **openapi `servers[0].url` 指向错误域名** — 指向 Web UI SPA（`builder.twin.so`），不是真实 API 网关（`build.twin.so`）。
2. **`POST /v1/agents/{id}/runs` 对任何 `run_mode` 值都 400** — 错误消息自相矛盾。
